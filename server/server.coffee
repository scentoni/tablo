...
Meteor.Router.add
  '/tables/:id.json': (id) ->
    ...
    if t?.publicq
      ...
      [404, {'Content-Type': 'text/html'}, 'Error! Bad id #']

# basic response
  '/tables/:id.csv': (id) ->
    t = Tables.findOne id
    if t?.publicq
      [200, {'Content-Type': 'text/csv'}, ContingencyTable.tocsv(t)]
    else
      [404, {'Content-Type': 'text/html'}, 'Error! Bad id #']
...
# canonical response
  '/tables/:id.tsv': (id) ->
    t = Tables.findOne id
    if t?.publicq
      [200, {'Content-Type': 'text/tsv'}, ContingencyTable.tocsv(t, '\t')]
    else
      [404, {'Content-Type': 'text/html'}, 'Error! Bad id #']
...
# basic response
  '/tables/:id.svg': (id) ->
    t = Tables.findOne id
    if t?.publicq and t.svg
      [200, {'Content-Type': 'image/svg+xml'}, t.svg]
    else
      [404, {'Content-Type': 'text/html'}, 'Error! Bad id #']
